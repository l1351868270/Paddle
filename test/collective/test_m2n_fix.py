...
def test_main(
    ...
    if rank >= a_start_rank and rank < a_start_rank + a_num_ranks:
        ...
        for idx in range(moe_layer_start_index * num_micro_batches, num_hidden_layers * num_micro_batches):
            ...
            print(f"rank: {rank}, attention: {mb_idx}_{layer_idx}", flush=True)
            if idx >= moe_layer_start_index * num_micro_batches + num_micro_batches:
                # a2e_isend_req.wait()
            ...
            event.current_stream_wait()
            print(f"rank: {rank}, layer_idx: {layer_idx}, mb_idx: {mb_idx}, a2e_send wait", flush=True)
            # if idx == num_hidden_layers * num_micro_batches - 1:
            #     a2e_isend_req.wait()
            #     print(f"rank: {rank}, layer_idx: {layer_idx}, mb_idx: {mb_idx}, a2e_isend wait", flush=True)            attention(num_tokens, hidden)
    ...
    if rank >= e_start_rank and rank < e_start_rank + e_num_ranks:
        ...
        for idx in range(moe_layer_start_index * num_micro_batches, num_hidden_layers * num_micro_batches):
            ...
            print(f"rank: {rank}, moe: {mb_idx}_{layer_idx}", flush=True)
            if idx == num_hidden_layers * num_micro_batches - 2:
                event.current_stream_wait()
                print(f"rank: {rank}, layer_idx: {layer_idx}, mb_idx: {mb_idx}, a2e_recv wait", flush=True)
            elif idx < num_hidden_layers * num_micro_batches - 2:
                event.current_stream_wait()
                print(f"rank: {rank}, layer_idx: {layer_idx}, mb_idx: {mb_idx}, a2e_recv wait", flush=True)
                (
                    packed_recv_x,
